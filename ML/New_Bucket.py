...
def predict(nulldata):
   ...
   # if :
    ...
    col=nulldata1.columns.tolist();
    col.append('avg_delay')
    Final=pd.concat([nulldata1,final_result.to_frame()], axis=1, ignore_index=True)
    Final.columns=col
...
def doc_id_bucket(doc_list):
   ...
   # if :
    ...
    #Final = nulldata1.merge(final_result.to_frame() , left_index=True, right_index=True)
    col=nulldata1.columns.tolist();
    col.append('avg_delay')
    Final=pd.concat([nulldata1,final_result.to_frame()], axis=1, ignore_index=True)
    Final.columns=col
